...
function updatesValues() {
  ...
  items.forEach((item) => {
    ...
    itemsOld[index].innerHTML = `$${(
      itemsOld[index].dataset.price * newQuantity
    ).toFixed(2)}`;
...
quantityButtons.forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    let newQuantity = eval(
      quantityItems[index].dataset.quantity + btn.dataset.type + "1"
    );
    if (newQuantity >= 1) {
...
function verifyInput(input) {
  let errorIcon = Array.from(input.parentNode.children).find((child) =>
    child.classList.contains("icon-validation")
  );
